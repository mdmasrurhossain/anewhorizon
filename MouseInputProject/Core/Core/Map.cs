    ...
    public class Map
    {
        ...
        private void DrawMap(Dictionary<Vector2, int> map, Texture2D atlas)
        {
            ...
            foreach (KeyValuePair<Vector2, int> tile in map)
            {
                ...
                Globals.SpriteBatch.Draw(atlas, new Rectangle((int)tile.Key.X * updatedPixelSize, (int)tile.Key.Y * updatedPixelSize, updatedPixelSize, updatedPixelSize), new Rectangle(tileX * pixelSize, tileY * pixelSize, pixelSize, pixelSize), BackgroundColor);
            }
        }
